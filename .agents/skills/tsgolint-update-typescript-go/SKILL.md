...
description: "Update the `typescript` submodule in the `oxc-project/tsgolint` repository, refresh the local `patches/*.patch` stack against the new upstream base, regenerate shims, open a PR, and watch CI to completion. Use when working on routine TypeScript bump PRs for TSGolint or when repeating the same workflow as prior submodule update PRs."
---

# Update TypeScript

Use this workflow for the standard TSGolint `typescript` refresh.
...
- advance the root `typescript` gitlink to a real upstream `microsoft/TypeScript` commit
...
- open a PR titled `chore: update TypeScript submodule`
- wait for CI to finish and report the result

2. Fetch the latest `microsoft/TypeScript` upstream state.

Check the current root gitlink and the current checkout inside `typescript/` separately.
...
- remember that the Go module and patched sources live under `tsc/`
...
Use `git format-patch --no-signature <upstream-base>..HEAD` from inside `typescript/`, then replace the root `patches/*.patch` files with the regenerated series.
...
- set the root `typescript` gitlink to the upstream commit being adopted
...
- Never commit a root gitlink pointing at a local-only patched `typescript` commit.
- Never discard unrelated root worktree changes unless the user explicitly asks.
- Treat the previous successful `chore: update TypeScript submodule` PR as the template for branch name, commit message, PR title, and expected diff shape.
