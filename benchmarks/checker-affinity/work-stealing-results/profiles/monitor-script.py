...
OUT = ROOT / 'results-steal-final-profiles'
...
def load():
    ...
    for line in raw.splitlines():
        ...
        if command.startswith(str(ROOT)) and name in ('pure-profile-v2.test', 'steal-final.test'):
            continue
        other_cpu += float(cpu)
        if name in BUILDERS:
...
event('monitor_policy', sample_interval_seconds=5, quiet_seconds=30, sustained_busy_seconds=10, other_cpu_threshold=200, phase='profile')
...
for attempt in range(first_attempt, first_attempt + 100):
    ...
    command = [sys.executable, 'benchmarks/checker-affinity/run.py',
               '--baseline', str(ROOT / 'pure-profile-v2.test'), '--candidate', str(ROOT / 'steal-final.test'),
               '--projects', str(ROOT / 'projects'), '--output', str(OUT),
               '--phase', 'profile', '--workers', '4', '--names', 'typescript']
    with (ROOT / f'steal-final-profiles-attempt-{attempt}.log').open('w') as log:
