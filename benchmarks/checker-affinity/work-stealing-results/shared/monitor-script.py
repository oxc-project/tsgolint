...
OUT = ROOT / 'results-steal-v3-shared'
...
def load():
    ...
    for line in raw.splitlines():
        ...
        if command.startswith(str(ROOT)) and name in ('baseline.test', 'steal-v3.test'):
            continue
        other_cpu += float(cpu)
        if name in BUILDERS:
...
def discard_affected(cutoff, attempt):
    ...
    shutil.copyfile(path, ROOT / f'quarantined-steal-v3-shared-attempt-{attempt}.csv')
...
for attempt in range(first_attempt, first_attempt + 100):
    ...
    command = [sys.executable, 'benchmarks/checker-affinity/run.py',
               '--baseline', str(ROOT / 'baseline.test'), '--candidate', str(ROOT / 'steal-v3.test'),
               '--projects', str(ROOT / 'projects'), '--output', str(OUT),
               '--phase', 'measure', '--workers', '4', '--names', 'typescript', 'vscode', 'typeorm', 'vue', '--scenarios', 'full-lint', 'full-check', 'ten-cold', 'ten-warm', '--samples', '10', '--resume-partial']
    with (ROOT / f'steal-v3-shared-attempt-{attempt}.log').open('w') as log:
