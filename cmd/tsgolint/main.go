...
import (
	...
	"github.com/typescript-eslint/tsgolint/internal/tsgolint"
...
func runMain() int {
	if len(os.Args) > 1 && os.Args[1] == "headless" {
		return tsgolint.RunHeadless(os.Args[2:])
	}
	...
	tsgolint.EnableVirtualTerminalProcessing()
	timeBefore := time.Now()

	if done, err := tsgolint.RecordTrace(traceOut); err != nil {
		os.Stderr.WriteString(err.Error())
		return 1
	} else {
		defer done()
	}
	if done, err := tsgolint.RecordCpuprof(cpuprofOut); err != nil {
		os.Stderr.WriteString(err.Error())
		return 1
	} else {
		defer done()
	...
	err = linter.RunLinterOnProgram(
		...
		func(sourceFile *ast.SourceFile) []linter.ConfiguredRule {
			return utils.Map(tsgolint.AllRules, func(r rule.Rule) linter.ConfiguredRule {
