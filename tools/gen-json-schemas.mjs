...
for (const schemaDir of schemaDirs) {
    ...
    try {
        execSync(`go-jsonschema  "${schemaPath}" -o "${outputPath}" -p ${path.basename(schemaDir)} --tags json --resolve-extension json`, {
            stdio: 'inherit',
        });
            ...
            for (const { fieldName, optionsType, defaultValue } of boolOrFields) {
                ...
                if (newContent !== content) {
                    ...
                    if (defaultValue !== undefined) {
                        ...
                        content = content.replace(defaultPattern, `plain.${goFieldName} = utils.BoolOrValue[${optionsType}](${boolDefault})`);
                    }
                    ...
                    if (!content.includes('"github.com/typescript-eslint/tsgolint/internal/utils"')) {
                        if (content.includes('import "github.com/go-json-experiment/json"')) {
                            ...
                            content = content.replace(/^import \(/m, 'import (\n\t"github.com/typescript-eslint/tsgolint/internal/utils"');
                        } else {
                    ...
                    modified = true;
                    console.log(`  Post-processed ${ruleName}: replaced ${goFieldName} interface{} with utils.BoolOr[${optionsType}]`);
                }
            }
        }
